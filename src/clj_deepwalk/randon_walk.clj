...
(defn deepwalk
  "Main function to run DeepWalk algorithm on a given graph and return the trained model."
  [graph num-walks walk-length vector-size window-size epochs]
  (let [walks (generate-random-walks graph num-walks walk-length)
        pairs (generate-pairs walks window-size)]
    (train-skipgram pairs vector-size window-size epochs)))


(defn get-embeddings
  "Extracts embeddings for each node from the trained model."
  [model nodes]
  (reduce (fn [embeddings node]
            (assoc embeddings node (.getWordVector model (str node))))
          {}
          nodes))
